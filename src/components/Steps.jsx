...
export default function HowItWorks() {
  return (
    <section className="bg-black py-16 px-4 text-center">
      <p className="md:text-xl text-base main-p font-semibold mb-2">How it works</p>
      <h2 className="text-4xl md:text-6xl  transition-all ease-in-out font-semibold mb-12 main- text-[#ffffff]">
        <span className='animate-pulse'>🚀</span> It Works In  3 Easy Steps 
      </h2>
      ...
      <div className="flex flex-col md:flex-row justify-center items-center gap-8  transition-all ease-in-out">
        {steps.map((step) => (
          <div
            key={step.id}
            className={`bg-gradient-to-tr from-[#833DFA] via-black hover:via-[#6c3dfa] border-[1px] border-[#823dfa4c] transition-all ease-in-out durat0 to-black text-white rounded-2xl p-6 w-72 transform  shadow-xl ${step.style}`}
          >
